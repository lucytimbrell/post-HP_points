...
Momocs::panel(SC_outlines_combined) # shows all outlines next to each other
...
panel(SC_outlines, main = "Sibudu Cave Outline data") # Visualization of points in their original orientation
...
plot.new()
gg <- PCcontrib(SC_pcashape, nax = 1:3, plot = FALSE)
gg$gg + 
...
(a | b | c) / d +
  plot_annotation(title = "Sibudan morphotypes", tag_levels = "a",
...
(x | y | z) / (l | m | n)  +
  plot_annotation(title = "PCA projection", tag_levels = "a",
                  theme = theme(plot.title = element_text(size = 24))) 
