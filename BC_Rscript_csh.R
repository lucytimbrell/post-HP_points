...
(a | b | c) / d +
  plot_annotation(title = "Member", tag_levels = "a",
                  theme = theme(plot.title = element_text(size = 24))) 
...
new_strat <- tidy.table %>% 
  ...
  mutate(New.strat = fct_relevel(New.strat, "2BS.UP", "2BS.LR", "2WA.UP", "2WA.MD", "2WA.LR", "3BS", "1RGBS")) %>%
  ggplot(aes(name, value, fill = New.strat)) +
  scale_fill_manual(guide = guide_legend(nrow = 1, byrow = TRUE), limits = c("2BS.UP", "2BS.LR", "2WA.UP", "2WA.MD", "2WA.LR", "3BS", "1RGBS"),values = c("#44AA99","#AA4499", "#6699CC", "#999933", "#888888", "#000000", "#000000")) +
  labs(y = "Score") +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "bottom",
...
(a | b | c) / d +
  plot_annotation(title = "Knapping Modality", tag_levels = "a",
                  theme = theme(plot.title = element_text(size = 24))) 
...
(a | b | c) / d +
  plot_annotation(title = "Retouch", tag_levels = "a",
                  theme = theme(plot.title = element_text(size = 24))) 
...
(a | b | c) / d +
  plot_annotation(title = "Tip orientation", tag_levels = "a",
                  theme = theme(plot.title = element_text(size = 24))) 
